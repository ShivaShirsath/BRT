...
export function VehicleAnalyticsPage() {
  ...
  const [data, setData] = useState<VehicleAnalytics | null>(null);
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      ...
      setLoading(true);
      try {
        const decodedVehicle = decodeURIComponent(vehicleNo);
        const { data } = await api.get(`/purchase/analytics/vehicle/${encodeURIComponent(decodedVehicle)}`);
        setData(data);
        setError("");
      } catch (err: any) {
        setError(err?.response?.data?.error || "Failed to load vehicle analytics");
      } finally {
        setLoading(false);
      }
    }
  ...
  const filteredMetrics = useMemo(() => {
    if (!data) return { trips: 0, bags: 0, weight: 0, value: 0 };
    let bags = 0;
    let weight = 0;
    let value = 0;
    ...
    return {
      trips: filteredTransactions.length,
      bags,
      weight,
      value
    };
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-muted-foreground font-semibold">Loading Vehicle Analytics...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background text-foreground p-6 flex flex-col items-center justify-center space-y-4">
        <Alert variant="destructive" className="max-w-md">
          <AlertDescription>{error || "Vehicle number is missing."}</AlertDescription>
        </Alert>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="sticky top-0 z-40 shrink-0 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 text-card-foreground shadow-sm py-4 px-6 flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-xl font-extrabold tracking-tight">Vehicle Analytics Dashboard</h1>
        </div>
        <div className="bg-[#1e3a8a] text-white px-4 py-1.5 rounded-full text-sm font-bold tracking-wider">
          {data.vehicleNo}
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="bg-gradient-to-br from-blue-50 to-white dark:from-slate-900 dark:to-card border-l-4 border-blue-500">
            <CardContent className="p-4 flex flex-col justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Trips</span>
              <span className="text-3xl font-extrabold text-blue-600 dark:text-blue-400 mt-2">{filteredMetrics.trips}</span>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-indigo-50 to-white dark:from-slate-900 dark:to-card border-l-4 border-indigo-500">
            <CardContent className="p-4 flex flex-col justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Bags Transported</span>
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-white dark:from-slate-900 dark:to-card border-l-4 border-amber-500">
            <CardContent className="p-4 flex flex-col justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Business Value</span>
        ...
        <Card>
          <CardHeader className="p-4 border-b">
            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Trip & Transaction History</CardTitle>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Farmer / Customer Name</TableHead>
                ...
                <TableBody>
                  {filteredTransactions.map((tx) => (
                    <TableRow key={tx.id} className="hover:bg-muted/50 transition-colors">
                      ...
                      <TableCell className="font-semibold text-slate-800 dark:text-slate-200">{tx.partyName}</TableCell>
                  ...
                  {filteredTransactions.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-6 text-muted-foreground">
                        No transactions recorded for this vehicle in this date range.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
